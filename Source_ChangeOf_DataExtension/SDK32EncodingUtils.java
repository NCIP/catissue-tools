...
public class SDK32EncodingUtils {
    ...
    {	
    	if(mapping==null)
    	{
    		mapping = getMapping(context, CASTOR_MARSHALLER_PROPERTY);
    	}
        return mapping;
    }
	...
	private static Mapping getMapping(MessageContext context, String mappingProperty) {
		...
		if (context != null) {
			...
			if (prop != null && !prop.trim().equals("")) {
				...
				LOG.info("SACHIN--mappingLocation "+mappingLocation);
			} else {
				try {
		...
		if (mappingCacheMap.containsKey(mappingLocation)) {
            ...
            LOG.info("SACHIN--mappingLocation contain in cache"+mappingLocation);
		} else {
			LOG.info("SACHIN--mappingLocation not contain in cache"+mappingLocation);
