...
public class AdvanceSearchForm extends ActionForm
{
				...
				if (validator.isOperator(opValue)) //IF the operator is a valid operator
				{
					...
					else
					{
						if (!SearchUtil.STRING.equals(bean.getValue())) //IF the datatype is not STRING
						{
							...
							else if (SearchUtil.NUMERIC.equals(bean.getValue())) //IF the data type is NUMERIC
							{
								//								Bug#1496 - '0' value should be allowed for search on fields that are double 
								...
								{
									value = (String) values.get(valKey + ":HLIMIT"); //Key for second field
	 ...
	 */
	public void setShowCalendarValues(Map showCalValues)
	{
	...
	private void validateEventParameters(Validator validator, ActionErrors errors)
	{
		...
		try
		{
			...
			eventParmDisNames = SearchUtil.getEventParametersDisplayNames(bizLogic, SearchUtil
		...
		for (int i = 1; i <= eventCounter; i++)
		{
			...
			if (validator.isValidOption(name) && validator.isValidOption(column))
			{
				...
				String fieldName = column.substring(column.indexOf(".") + 1, column
						.lastIndexOf("."));
				String dataType = column.substring(column.lastIndexOf(".") + 1);
				String errorKey = name + "." + fieldName;

				if (eventParmDisNames != null && eventParmDisNames.get(errorKey) != null)
				{
					errorKey = (String) eventParmDisNames.get(errorKey);
				}

				if (value.trim().equals(""))
				{
					errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("errors.item.missing",
							errorKey)); //NOPMD - ActionError instantiation
				}
				...
				else if ("double".equals(dataType) && !validator.isDouble(value))
				{
					errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("errors.item.format",
							errorKey)); //NOPMD - ActionError instantiation
				}
