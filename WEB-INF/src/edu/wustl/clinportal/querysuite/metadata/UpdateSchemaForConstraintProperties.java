...
public class UpdateSchemaForConstraintProperties
{
			...
			DAOException
	{
		...
		try
		{
			...
			while (resultSet.next())
			{
				cnstrPropId = resultSet.getLong(1);
				srcEntityKey = resultSet.getString(2);
				tgtEntityKey = resultSet.getString(3);
				srcCnstrName = resultSet.getString(4);
				associationId = resultSet.getLong(6);
				categoryAssonId = resultSet.getLong(7);
				Vector v = new Vector();
				v.add(cnstrPropId);
				v.add(srcEntityKey);
				v.add(tgtEntityKey);
