...
public class HQLCoreQueryProcessor extends LazyCQLQueryProcessor {
	...
	/**
	 * This method is added to authenticate caTissue User before calling Appservice 
	 * search
	...
	private void applyUserSecurity() throws QueryProcessingException
	{
        ...
        try
        {
            java.io.InputStream inputStream = new FileInputStream(new File((new StringBuilder()).append(System.getProperty("user.home")).append("/catissueservice/credentials.properties").toString()));
            Properties properties = new Properties();
            properties.load(inputStream);
            userName = properties.getProperty("user.name");
            password = properties.getProperty("password");
            keystoreFilePath = properties.getProperty("keystoreFilePath");
        ...
        {
            throw new QueryProcessingException((new StringBuilder()).append("Error logging into caTissue systyem: ").append(e.getMessage()).toString(), e);
        }
        catch(IOException e)
        {
            throw new QueryProcessingException((new StringBuilder()).append("Error loading caTissue credentials: ").append(e.getMessage()).toString(), e);
        }
        try
        {
        	ClientSession securitySession = ClientSession.getInstance();
            System.out.println("\nLogging using caTissue credentials...");
            boolean loggedIn = securitySession.startSession(userName, password);
            if(!loggedIn)
                throw new QueryProcessingException("CSM security did not log in user");
        }
        catch(ApplicationException ex)
        {
            throw new QueryProcessingException((new StringBuilder()).append("Error initializing CSM security: ").append(ex.getMessage()).toString(), ex);
        }
