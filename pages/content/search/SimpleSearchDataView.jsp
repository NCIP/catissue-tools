...
<%@ page import="edu.wustl.clinportal.actionForm.AdvanceSearchForm"%>
...
<%
	...
	List columnList = (List) session.getAttribute(edu.wustl.common.util.global.Constants.SPREADSHEET_COLUMN_LIST);
	if(columnList==null)
		columnList = (List) request.getAttribute(edu.wustl.common.util.global.Constants.SPREADSHEET_COLUMN_LIST);
...
<html:hidden property="checkAllPages" value=""/>	
	...
	<%
		if(dataList == null && pageOf.equals(edu.wustl.query.util.global.AQConstants.PAGEOF_QUERY_RESULTS))
		{
		%>
	...
	%>
		...
		<%
		if(pageOf.equals(edu.wustl.query.util.global.AQConstants.PAGEOF_QUERY_RESULTS))
		{			
			String []selectedColumns=form.getSelectedColumnNames();
		%>
		
		<tr id="hiddenCombo" rowspan="4">
			<td class="black_ar_new" colspan="4">
	<!-- Mandar : 434 : for tooltip -->
...
<!--  **************  Code for New Grid  *********************** -->
		...
		<td width="90%">
			...
			<tr>
					<td width="5%" nowrap valign="top" class="black_ar_new">
					<input type='checkbox' name='checkAll2' id='checkAll2' onClick='checkAllOnThisPage(this)'>
						<span class="black_ar_new"><bean:message key="buttons.checkAllOnThisPage" /></span>
						<input type='checkbox' name='checkAll' id='checkAll' onClick='checkAllAcrossAllPages(this)'>
						<span class="black_ar_new"><bean:message key="buttons.checkAll" /></span>
					</td>
					<%
					...
					<%if(pageOf.equals(edu.wustl.query.util.global.AQConstants.PAGEOF_QUERY_RESULTS)){%>
						...
						<span class="black_ar_new"><bean:message key="buttons.defaultSpecimenView" /></span>&nbsp;
					<%}else{%>
						&nbsp;
					<%}%>
					</td>
